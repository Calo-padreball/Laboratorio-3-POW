...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (savedPosts) {
        try {
            ...
            mostrarPost(postsArray);
            statusArea.textContent = "Cargado desde localStorage.";
            statusArea.classList.add("status-message--success");
        } catch (error) {
            console.error("El JSON de localStorage estaba corrupto. borrando...", error);
...
//Variables
const buttonCargar = document.getElementsByClassName("btn primary-btn");
...
postForm.addEventListener("submit", function(evento) {
    ...
    if(userId >= 1 && userId <= 10){
        console.log("Dato valido, enviando...");
        statusArea.textContent = "Cargando...";
        statusArea.classList.add("loading");
        obtenerPosts(userId);
    } else {
        console.log("Dato inválido en submit");
        statusArea.textContent = "Error: El ID debe ser entre 1 y 10";
    }
});
...
async function obtenerPosts(userId) {
    try {
        ...
        statusArea.textContent = "Éxito";   
        statusArea.classList.remove("loading");
        console.log(datos);
        mostrarPost(datos);
    } catch (error) {
        console.error("Error en la petición fetch:", error);
        statusArea.textContent = "Error al cargar los datos";
        statusArea.classList.remove("loading");
    }
}
...
function mostrarPost(arregloPost) {
    postsList.innerHTML = " ";
    arregloPost.forEach(post => {
    ...
    localStorage.setItem(POSTS_DATA_KEY, JSON.stringify(arregloPost));
}
